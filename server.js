...
app.get('/api/assign-code', (req, res) => {
  try {
    ...
    if (type !== 'email2' && type !== 'email3') return res.status(400).json({ error: 'Type must be email2 or email3' });
...
app.get('/api/stats', (req, res) => {
  ...
  res.json({ email2: { total: EMAIL2_CODES.length, used: email2Used, available: EMAIL2_CODES.length - email2Used }, email3: { total: EMAIL3_CODES.length, used: email3Used, available: EMAIL3_CODES.length - email3Used } });
});
